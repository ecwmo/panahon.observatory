    ...
    <div class="flex flex-col items-center space-y-2 mx-12 md:w-full">
      ...
      <div class="flex flex-col w-1/2">
        <fieldset class="w-full mb-2">
          <input type="range" name="decade" :min="minDecade" :max="maxDecade" step="10" x-model.number="activeDecade"
            class="relative w-full h-2 -bottom-1 m-0 cursor-pointer">
          <svg class="w-full px-2 overflow-visible stroke-current text-gray-500" role="presentation" height="5"
          ...
          <svg class="w-full px-2 overflow-visible mt-1" role="presentation" height="10"
            xmlns="http://www.w3.org/2000/svg">
            <template x-for="(d, i) in decades" :key="d[0]">
              <text class="text-xs font-semibold" :x="`${100*i/(decades.length-1)}%`" y="10" text-anchor="middle"
                x-text="d[0]"></text>
            </template>
          </svg>
        </fieldset>
