  ...
  <div class="flex py-4 justify-center">
    <div class="flex flex-col bg-skin-body-fill-inv text-skin-inverted shadow-md rounded px-8 pt-6 pb-8 mb-4">
      <div v-if="publishedView">
        ...
        <p class="p-1 mb-3">
          Click
          <a :href="route('report')" class="underline text-skin-link hover:text-skin-link-active">here</a>
        </p>
      </div>
      <div v-else-if="publishView">
        <h4 class="p-4 flex justify-center text-3xl font-medium">Upload success!</h4>
        <p class="p-1 mb-3">
          Preview the document
          <a
            :href="route('report?view=draft')"
            target="_blank"
...
<script setup lang="ts">
  ...
  const publishedView = computed(() => props.pageMode === 'published')
  const publishView = computed(() => props.pageMode === 'publish')
